...
held_out = pd.read_csv('mnist_test.csv', header = None)
held_out_binario = pd.read_csv('mnist_test_binario.csv')
#%%
#################################Ejercicio 1 ################################################
...
primeraFila = df.iloc[:, 2:28].values.flatten()
primeraColumna = df.iloc[:, 1::28].values.flatten()
ultimaFila = df.iloc[:, 758:785].values.flatten()
ultimaColumna = df.iloc[:, 28::28].values.flatten()

borde_count = pd.Series(
    [*primeraFila, *primeraColumna, *ultimaFila, *ultimaColumna]
)

borde_count.value_counts()[0]/borde_count.value_counts().sum()
...
# Max con 3 vecinos
# %%
# Entrenamos con los píxeles elegidos y k = 3 (held out binario)
X = df.iloc[:,[496,293,371,379,435]]
Y = df.iloc[:, 0]

X_held_out = held_out_binario.iloc[:,[496,293,371,379,435]]
Y_held_out = held_out_binario.iloc[:, 0]

model = KNeighborsClassifier(n_neighbors = 3)
model.fit(X, Y)
Y_pred = model.predict(X_held_out)
Y_pred_train = model.predict(X)
acc_held_out = metrics.accuracy_score(Y_held_out, Y_pred)
acc_train = metrics.accuracy_score(Y, Y_pred_train)
# acc_held_out = 0.9938534278959811
# acc_train = 0.9957362810896171
...
# %%
# Evaluación held out en árboles de decisión

df = pd.read_csv('mnist_desarrollo.csv', header = None)
X = df.iloc[:,1:]
Y = df.iloc[:, 0]

X_held_out = held_out.iloc[:,1:]
Y_held_out = held_out.iloc[:, 0]

depth = 11

clf_info = tree.DecisionTreeClassifier(criterion = 'entropy', max_depth= 11)
clf_info = clf_info.fit(X, Y)

# Predicciones para el set test
pred_test = clf_info.predict(X_held_out)
  
# Calculo accuracy usando el set de held out
score_test = metrics.accuracy_score(Y_held_out, pred_test)

# Calculo accuracy para el set desarrollo
pred_train = clf_info.predict(X)
score_train = metrics.accuracy_score(Y, pred_train)
print('Accuracy en held out:', score_test, '\n')
